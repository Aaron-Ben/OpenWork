...
| **无人值守运行** | 当前阶段不是目标。人始终在环路里；被自动放行的是可证明只读与工作区内的文件改动，不是"整轮不看"。**唯一的例外是子 Agent**——它没有人可问，因此 `Ask` 一律 Deny，只能跑可证明只读的东西，见 §6.6 |
...
- token 以 `~` 开头（波浪号展开）；
- 未加引号的 token 同时含 `{` 与 `,` 或 `..`（大括号展开）；
- 含反引号；
- `rg` / `grep` 的模式里含换行或回车。

只有大括号这一条区分引号来源：单引号或双引号内的 `{n,m}` 是传给程序的字面量，不会被 shell 展开。`$` 和反引号在双引号内仍有可展开形态；`~` 在引号内虽不展开，也和前两者一样继续充当 token 解码出现 parser differential 时的兜底。引号元数据只能收窄已经确定没有 shell 展开的 brace 子句，不能撤掉这三道守卫。
...
- **cwd 只追踪一个封闭子集。** 唯一例外是顶层扁平 `program` / `list` 的第一个、也是唯一一个 cwd 变更为 `cd <静态字面量>`，且目标词法规范化后仍在工作区内。此时后续相对路径只把拼接基点换成该目录；路径边界和 PATH 资格检查仍使用真实工作区。`pushd` / `popd` / `env -C`、多个 cwd 变更、动态目标、管道或子 shell 等其余形态仍整条不合资格。
...
**⑤ 脚本内出现 cwd 变更** —— 默认仍是整条不合资格。只为一种可封闭证明的前缀追踪 cwd：顶层扁平 `program` / `list` 以唯一的 `cd <静态字面量>` 开头，目标词法规范化后在工作区内。后续相对路径从该目标拼接，但规则层判断是否越界、以及程序名的 PATH 解析，始终以真实工作区为边界；因此 `cd <工作区>/packages && cat ../../etc/passwd` 仍会落到问。任何额外 cwd 变更、动态目标、管道、子 shell、函数、循环或条件都退回整体不合资格。
...
### 6.4 回看：自动放行必须在 Trace 里认得出来

`default` 下大量 `bash` 调用静默执行，`acceptEdits` 下工作区内的写入也静默执行。**这些不出卡片，所以用户当场看不见。**

回看的地方是 **Trace**，不是另建的面板。Tool Span 上已经带着回答"这条为什么没问我就跑了"所需的全部字段（§7），因此这里只有一条要求：

> **时间线上必须一眼分得出哪些调用没经过用户。**

四类要互相可区分：

| 类别 | 判据 | 为什么单独标 |
|---|---|---|
| **自动放行** | `permissionDecision = allow` 且来源不是 `user` | 用户当场没看见它发生 |
| **静默拒绝** | `permissionDecision = deny` 且来源是 `builtin` | 同样不出卡片。**它是唯一"有人试图越界"的信号**，而用户最看不见它 |
| 用户批准 | `allow` + `user` | 用户当场看见过 |
| 用户拒绝 | `deny` + `user` | 同上 |

自动放行的那一类**还要能看出来源**（§7 的五个取值）：`readonly_proof`（我们核对过参数）与 `mode`（模式开着所以放行）是很不一样的两件事，合并之后用户判断不了该担心哪一条。来源是 `readonly_proof` 时要能看到 `readonlyProofKey`——**判定表是我们自己背书的东西，写错时按条目反查是唯一的补救**。

**不做独立的会话活动面板，也不做会话授权列表。** 授权产生的效果在 Trace 里看得见（`permissionDecisionSource = session_grant` 的那些 Span 带着 `permissionRuleId`，可还原到是哪一次批准），再单独列一遍是冗余。用户在按下按钮时看到的授权范围由 §5.2 的按钮文案负责，那是它该被看清的时刻。

### 6.5 缺口必须可见

Trace 是有损的（trace.md §10：队列满时直接丢，批量写失败丢整批），所以上面这份标记**可能少标几条**。

单独看这是致命的——**一个会漏报的漏报检测器比没有更糟**：用户扫一眼觉得没事，而漏掉的那条恰恰是出问题的那条。

救它的是完整度的算法（trace.md §12）：`expected` 来自 `turns.tool_call_count`，**由业务写入路径维护，与 Trace 写入路径互不依赖**。跑了 20 次工具调用只落了 18 条 Span，状态就是 `partial`。

于是契约不是"每条都在"，而是：

> **单条记录可能丢，但丢没丢一定看得出来。**

**因此 `partial` / `none` 在界面上必须显著区别于 `complete`。** 这一条不是观感问题：标记让人更信任这个视图，而一个更被信任的视图在漏报时危害更大。

### 6.6 非交互 Session：没有人可问时 `Ask` 落到哪

子 Agent（[multi-agent.md](multi-agent.md)）是**没有人在环路里**的 Session。`SessionRuntimeConfig` 因此带一个运行环境属性：

```rust
pub enum SessionApproval {
    Interactive,      // 根会话：Ask 挂起，等用户决定
    NonInteractive,   // 子 Agent：Ask 立即 Deny
}
```

**这不是第三个 `PermissionMode`。** 模式是"用户选择的审批尺度"（§3.5），而这里既不是用户选的、也不是尺度，是"没有人"。混进模式枚举会让 §3.5 与 §6.3 那套模型全部失真。

改变的**只有** `Ask` 的落地方式，其余一个字节不动：

| 环节 | 非交互 Session 的行为 |
|---|---|
| 规则集（内置 + 会话授权） | 完全相同 |
| 只读判定（§2.3） | 完全相同，`git log` / `rg` / `ls` / `cat` 照常自动放行 |
| 两条硬 `deny` 与敏感路径 `ask` | 完全相同 |
| 归约与会话授权（§4.6.1） | 不适用——没有卡片，也就没有「本会话允许」 |
| `Ask` | 立即产出 `denied` 的 Tool Result，不生成卡片、不挂起 |

Deny 的文案必须**可操作**，不能只说"被拒绝"：告诉模型这是子 Agent 无法请求授权所致，改用可证明只读的命令即可继续。否则模型会反复重试同一条命令直到耗尽 `max_model_calls`。

这条设计让"子 Agent 拿着 `bash`"是安全的：它能跑的恰好是 §2.3 明确背书的那个集合，**没有新开任何口子**。子 Agent 的工具池里本来也没有 `write` / `edit`。

§7 的可追溯要求同样适用：非交互 Deny 也要在 Tool Span 上记下 `AskSource`，否则"explorer 为什么什么都没查出来"无法复盘。
...
`permissionModeOrigin` 同理。`acceptEdits` 下的文件写入来源是 `mode`，但"用户主动切过去的"和"卡片上顺手点过去的"在复盘时是两件事——后者意味着那次批准的真实覆盖面远大于当时卡片上那一个文件（§4.6.2），必须能从 Trace 里认出来。

权限**不是独立 Span**：Tool Span 包围完整生命周期，等待时间作为属性记录。

**保证下在完整度上，不下在单条记录上。** Trace 是有损的（trace.md §10），单条属性可能随整条 Span 一起丢；能保证的是**丢没丢一定看得出来**——完整度的 `expected` 来自 `turns.tool_call_count` 这个独立参照物，与 Trace 写入路径互不依赖（trace.md §12）。因此界面必须在 `partial` / `none` 状态下显著提示（§6.5），否则这条保证不成立。字段缺失时 UI 应显示"来源未知"，而不是留白。
...
73a. Trace 时间线上，自动放行 / 静默拒绝 / 用户批准 / 用户拒绝**四类互相可区分**；
73b. 自动放行的**五种来源可区分**：`builtin` / `readonly_proof` / `mode` / `mode_fs_command` / `session_grant`。特别地，`mode` 不得并入 `builtin`——前者表示"因为你开了 acceptEdits"，后者表示"本来就允许"；
73c. 来源为 `readonly_proof` 时能看到命中的判定表条目；来源为 `session_grant` 时能从 `permissionRuleId` 还原出是哪一次批准授的权；
73d. 每条记录能看出当时的模式（`permissionMode`）与模式的来源（`permissionModeOrigin`）；
73e. 完整度为 `partial` / `none` 时，界面显著区别于 `complete`（§6.5）；
