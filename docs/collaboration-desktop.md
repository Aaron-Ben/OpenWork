...
**静音是一个 `(房间, 成员)` 上的开关，两种含义共用一列。** `collab_room_members.muted` 在用户那一行的意思是"别把这个房间算进我的未读总数"（上一段），在某个 Agent 那一行的意思是"这个房间不要唤醒它"（[collaboration.md §7.1](collaboration.md)）。界面要同时提供这两个开关：房间行上的静音管自己，房间成员列表里每个 Agent 旁的静音管那个 Agent。

后者不是可有可无的装饰——它是 Agent **不被允许自己 `leave` 房间**的前提。"这房间跟我无关"的出口必须存在，只是它归用户按。
...
OpenCode 事件驱动的名册 + HELD 反馈 + 分页的长房间表现。Agent 管理页同时提供全局 triage provider / model 配置：候选只来自已启用的 OpenWork provider / model，未配置时明确显示人类消息 fail-open，保存后下一次裁决立即生效，无需重启 daemon。
...
18. 用户能从空房间建立第一块看板，新增列时显式设置 `isDone`，再建卡并指派给某个 Agent；卡片出现在看板且房间收到系统消息；
...
24. 日志抽屉能区分一轮的三种结果（`acted` / `silent` / `unpublished`，见 [collaboration.md §8.3](collaboration.md)）——"它想了半天什么都没说"和"它判断这不是叫它"在界面上不是同一件事；`unpublished` 要显眼，那是异常不是常态。

### 横切验收

与 [collaboration.md §16 横切验收](collaboration.md) 同一组，这里只列前端可见的部分。

25. 房间行的静音开关可用，静音后该房间不计入 Rail 与侧栏按钮的未读总数；
26. 房间成员列表里能对单个 Agent 静音，静音后它不再被该房间唤醒；
27. 用户被拉进 / 移出房间，以及 Agent 被拉进 / 移出，在房间流里出现 `system` 消息；
28. 打开一个 Agent 刚被加入的房间，它的首次回应不表现为"把三个月的历史重读了一遍"。

### 贯穿全程

29. 前端不出现任何协作语义判定（HELD、去重、认领的裁决）——界面只显示 daemon 给的结果；
30. 新增的纯函数、reducer 与 store 有与现有同强度的 vitest 覆盖；
31. 未读总数在房间列表、Rail、侧栏按钮三处一致，且只有一个来源；待审批数同理。
