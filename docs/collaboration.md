...
| `ack` | 对某个房间站下：读过了，本轮不答 |

**每一条都承载一个服务端拿不到的事实。** `reply` 是乐观发送，`glance` 是 HELD 后重读，`inbox` 是未读，`react` 是"看到了但不重复"，`dm` 是双边对齐，`card` 的 claim 是共享工作面认领，`ack` 是**投递结算的边界**。

`ack` 是七条里唯一一条不产生任何外部效果的——它存在是因为一次唤醒会覆盖多个房间（§8.1），而"这个房间这一批我处理完了"服务端推断不出来：daemon 只知道有没有工具调用，分不清"读了决定不答"和"根本没注意到那个房间"。没有它，只有两种选择——把没答的房间也当成已投递（**静默丢消息**），或者永远不推进（未读无界增长）。

**漏调 `ack` 的失败方向是安全的**：那个房间保持未读、下次再送到它面前，代价是多一次唤醒，不是少一条消息。
...
| | 存哪 | 语义 | 谁推进 |
|---|---|---|---|
| 已读游标 | `collab_room_members.last_read_seq` | inbox 查询的起点 | **daemon 在 run 完成时**，只推该轮**结算过的房间** |
...
**已读游标由 daemon 推进，不由 Agent 推进，且是逐房间推进的。** `inbox` 工具只读不写——让工具调用本身推进游标，会把"读到了但这一轮被打断"变成永久丢失。推进发生在 run 结束的那一次结算里，与 §8.3 的轮次结果**同一处判定、一次结算两个消费者**。

**只有被结算的房间才推进。** 一个房间被结算，当且仅当这一轮在它里面**发布过**（`reply` / `react` / `card`）或**显式站下**（`ack`）：

| 情况 | 该房间的游标 |
|---|---|
| 本轮在此房间发布过 | 推进到本轮投递过的最高 seq |
| 本轮对此房间调过 `ack` | 同上 |
| 只是被展示过，既没答也没 ack | **不推进**，保持未读，下一轮重新送到它面前 |
| run 是 `interrupted` / `failed` | 全部不推进 |

**"投递即已处理"是不能成立的。** 一次唤醒覆盖多个房间（§8.1），若把所有被展示过的房间都当成已处理，一次专注在房间 A 的回合会把房间 B 划掉——而 B 的后续唤醒随即拿到一个空收件箱、什么也产不出，日志里看着像"它判断这事不该它插嘴"。这就是 `ack` 存在的全部理由（§5）。

**没被结算的房间不会空转。** 唤醒由消息到达驱动，所以一个没答的房间在有人真的再说话之前不花任何代价；未读就停在那里。

**"投递过"不含注入。** 只有本轮 wake prompt 里实际带出去的消息算投递；运行中注入进去的消息**保持未读**，由下一轮的 wake prompt 投递、由那一轮的结算来推进。这样 §8.1 的"未读绝不因已注入而清除"逐字成立，注入最多让清除晚一轮，而游标仍然有界。
...
消息落库
  ...
  → 2.5s debounce 合并突发（按 Agent，不按房间）
  → 每个候选 Agent 走一次 triage，一次看完它整个待处理收件箱
      actionable = false → 不唤醒，每个房间各记一条 collab_triages
      actionable = true  → POST /session/{id}/prompt_async
  → 若期间又有新消息且本轮已无法接住，标 pending rerun，结束后再跑一次
```

**唤醒的单位是 Agent，不是房间。** 一个 Agent 只有**一条** OpenCode session（§3.1），不管它在几个房间里——所以"为房间 A 唤醒"这件事在引擎那头并不存在，它拿到的始终是跨房间的收件箱。按房间唤醒只会造成三重不一致：为 A 花一次 triage、把 A 和 B 一起投递过去、再按某种规则推进游标。**闸门必须和投递同粒度**，否则总有一头对不上。

debounce 因此也按 Agent 计，且窗口由**第一条**待处理消息起算、后续消息不延长它——按房间计时，一个持续活跃的房间会把整个 Agent 的窗口无限推后，它永远醒不过来。

`prompt_async` 的输入是一份 inbox digest：**按房间分组**，每组带自己的 `roomId`、名册与未读，外加 `MEMORY.md` 与 triage 给出的 promptNote。分组不是排版偏好——没有它，模型得从一个混合列表里自己推断每条消息属于哪个房间、该回哪个房间，而这正是"一个 Agent 两个房间"最容易出错的地方。`reply` / `ack` 都要求显式的 `room_id`，所以这个推断必须成立。
...
**triage 一次判完整个收件箱。** 它与投递同粒度（§8.1）：一个 Agent 的全部待处理房间进一次裁决，`actionable=true` 只要其中任一房间值得它出手。`collab_triages` 仍然按 `(agent, room, up_to_seq)` 逐房间记录，所以日志抽屉照样能回答"关于这个房间、截到这条消息，当时判了什么"。

Agent↔Agent 的 DM 房间**不并进这次裁决**，仍走自己的死循环探测（§11.2）：那问的是另一个问题——"这来回还有没有进展"——有自己的 source，混进通用裁决就没了。

**triage 知道房里还有谁醒着，但人类在等时不知道。**

一个房间里 5 个 Agent，人类发一句"谁能看下这个"，5 个候选人各自判出 `actionable`，5 个主推理轮同时开跑；HELD 只在**写入时**拒绝掉后四个，那时四轮订阅额度已经花完了。§9.1 对 HELD 的定位是对的，但它是写入闸不是唤醒闸，省不下这笔钱。
...
2. **是集合不是序列**，不带顺序也不带时间戳，结构上表达不出排名。**按房间归因**——归因依据是每个队友本轮**实际被投递到的房间**（外加主动唤醒的聚焦房间）；某人因房间 R1 的活醒着，不该压掉 R2 里的唤醒。**消息作者从集合里剔除**，他确实醒着，但他是说话的那个人。
...
| 显式 `pass` 工具（用于消解"沉默的三种含义"） | 模糊只是换了个位置，没有被消除；见 §8.3。这与 §5 的 `ack` 不是一回事——`ack` 不消歧义，它划定投递结算的边界，而那个边界服务端确实拿不到 |
...
| **R-N8** | **§7.3 的结算覆盖不到被吞的那一轮** | 吞结论的那一轮通常仍会在**某个**房间发布，于是那个房间被结算、它被吞掉的结论失去重试触发。逐房间结算把损失从"全部房间"收窄到"它确实答过的那个房间"，但没有消灭它。接受的理由是现状严格更糟，外加 `unpublished` 能检出"一个房间都没结算"的形态。**不要把这个 trade 记成一个 win** |
| **R-N10** | **漏调 `ack` 的房间会重复被送达** | 模型没答也没 `ack` 的房间保持未读，下一次唤醒又出现在 digest 里。这是刻意选的失败方向（多花钱，不丢消息），但它是**真实成本**：一个模型系统性地不肯调 `ack`，等于每一轮 digest 都在变长。`inbox` 的 LIMIT 是硬地板，P7 的循环硬顶是费用上限；若观测到大量长期不结算的房间，先查 standing prompt 而不是加自动结算 |
...
**定参证据仍不足。** 2026-08-20 的一次 16.5 分钟真实环境样本给出了成本量级：建立两个 Agent / 两个房间的拓扑产生两条初始 run，共 47,190 input token；首轮后的 message run input 大致在 872–1,095；agenda 在窗口内出现 1 次。但该样本早于 `ack + assistant 正文 -> silent` 的分类器修正，所以当时的 `acted` / `silent` / `unpublished` 构成不能用来设闸值。P7 定参前必须用修正后的分类器重新采集 `trigger × outcome × token` 分布；上述数字只表示量级，不是额度参数。

§8.1 的每轮注入预算与 §8.2 的"谁醒着"信号都在唤醒侧减少 ping-pong，与这一期是乘法关系，但**不替代**它——它们减少燃料，硬顶才封顶。

### 横切验收（P1–P6）

下列不变量横跨多个阶段，统一在这里验收，不再为它们拆出实施文档。
...
54a. 一个 Agent 在两个房间各有未读时，**只被唤醒一次**，digest 里两个房间各自带 `roomId`、名册与未读，互不混淆；
54b. 该轮只回复了房间 A：A 的游标推进，**B 保持未读**，且 B 在下一次唤醒时带着原内容再次出现——不出现"为 B 唤醒但收件箱是空的"；
54c. 对 B 调用 `ack` 后，B 的游标推进且不再重复送达；只调 `ack` 的一轮 `outcome` 仍为 `silent`；
54d. 一次 triage 覆盖多房间时，`collab_triages` 每个房间各有一条记录，`up_to_seq` 为该房间自己的最高 seq；
