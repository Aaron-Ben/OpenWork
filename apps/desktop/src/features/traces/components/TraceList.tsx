import { Activity, Bot, Minimize2, Wrench } from 'lucide-react'
      ...
      {items.map((item, index) => {
        ...
        return (
          <button
            ...
            className={`flex w-full items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-paper-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-clay/35 ${index > 0 ? 'border-t border-line' : ''}`}
            onClick={() => onOpen(item)}
          >
            ...
            <span className="hidden shrink-0 items-center gap-3 text-[11px] text-ink-faint lg:flex">
              {isTurnless ? null : (
                <>
                  <span className="inline-flex items-center gap-1"><Bot size={12} />{t('activity.modelCalls', { count: item.modelSubmissionCount })}</span>
