...
export function buildTraceListQuery(
  ...
  const input: TraceListQuery = { limit, offset }
  const normalized = query.trim()
  if (normalized) input.query = normalized
...
export function TraceSettings() {
  ...
  async function load(offset: number, append: boolean) {
    ...
    setError(null)
    try {
      const page = await sessionsApi.traceList(
        buildTraceListQuery(debouncedQuery, status, offset),
      )
      if (requestId !== requestSequence.current) return
      setSummaries((current) => (append ? [...current, ...page.items] : page.items))
      setNextOffset(page.nextOffset)
    } catch (reason) {
      if (requestId === requestSequence.current) setError(resolveErrorMessage(reason))
    } finally {
  ...
  useEffect(() => {
    void load(0, false)
    // `load` intentionally captures the current server-side filter values.
    ...
    <div className="relative h-full bg-paper">
      <div className="h-full overflow-auto">
        <div className="mx-auto w-full max-w-5xl p-8 max-[640px]:p-5">
          ...
          <div className="mt-5">
            <TraceSettingsList summaries={summaries} onOpen={setSelectedTurnId} />
          </div>
  ...
  return (
    <div data-settings-trace-list="true" className="overflow-hidden rounded-2xl border border-line bg-surface">
      {summaries.map((summary) => {
        const sessionTitle = summary.sessionTitle ?? t('settings.trace.unknownSession')
        const project = projectName(summary.workingDir) ?? t('settings.trace.unknownProject')
        const title = summary.inputPreview ?? sessionTitle
        return (
          <button
            key={summary.turnId}
            type="button"
            data-trace-row={summary.turnId}
            aria-label={`${t('settings.trace.openDetail')}: ${title}`}
          ...
          >
            ...
            <span className="min-w-0 flex-1">
              <span className="flex min-w-0 items-center gap-2">
                <span className="truncate font-sans text-sm font-medium text-ink">{title}</span>
                ...
                {summary.dataCompleteness !== 'complete' ? (
                  <span className="shrink-0 rounded-md bg-status-warning-soft px-1.5 py-0.5 font-sans text-[11px] text-status-warning-ink">
                    {t(`trace.completeness.${summary.dataCompleteness}`)}
                  </span>
                ) : null}
              </span>
              <span className="mt-1 flex flex-wrap gap-x-3 gap-y-1 font-sans text-xs text-ink-faint">
                ...
                {summary.errorCount > 0 ? <span className="text-status-danger-ink">{t('settings.trace.errorCount', { count: summary.errorCount })}</span> : null}
                {summary.diagnosis.reason !== 'healthy' ? (
                  <span className="text-status-danger-ink">{t(`trace.diagnosis.${summary.diagnosis.reason}`)}</span>
                ) : null}
              </span>
            </span>
            <span className="hidden shrink-0 text-right font-sans text-xs text-ink-faint sm:block">
              ...
              <span className="mt-1 flex items-center justify-end gap-1">
                {summary.errorCount > 0 ? <AlertCircle size={12} className="text-status-danger-ink" /> : <Clock3 size={12} />}
