use openwork_core::{
    ...
    TraceContentPolicy, TracePayloadSlot, TracePayloads, TraceRecorder, TraceSignal, TraceStatus,
...
async fn oversized_payloads_are_truncated_and_the_database_enforces_truncation_metadata() {
    ...
    let recorder = PostgresTraceRecorder::spawn_with_content_config(
        storage.pool().clone(),
        TraceContentConfig::new(TraceContentPolicy::Full, 128, 30).unwrap(),
    );
    recorder.record(finish_compaction(root));
    recorder.record(finished_model(
        model_started(
...
#[tokio::test]
async fn content_policy_off_and_compaction_only_leave_spans_and_tokens_intact() {
    let Some(storage) = storage().await else {
        return;
    };

    let (off_session, off_turn) = create_turn(&storage, "session-payload-off").await;
    let off_marker = unique("off-request");
    let off_request = request(&off_marker);
    let off_span = model_started(
        &off_session,
        Some(&off_turn),
        off_turn.as_str(),
        None,
    ...
    let off_span_id = off_span.span_id.clone();
    let off_recorder = PostgresTraceRecorder::spawn_with_content_config(
        storage.pool().clone(),
        TraceContentConfig::new(TraceContentPolicy::Off, 1_024, 30).unwrap(),
    );
    off_recorder.record(finished_model(off_span, Some(json!({"text": "off"}))));
    assert!(off_recorder.flush_turn(&off_turn).await.flushed);
    let off_row: (String, Option<i64>, Option<i64>) =
        sqlx::query_as("SELECT status, input_tokens, output_tokens FROM trace_spans WHERE id = $1")
            .bind(&off_span_id)
            .fetch_one(storage.pool())
            .await
            .unwrap();
    assert_eq!(off_row, ("succeeded".to_string(), Some(11), Some(7)));
    let off_payload_count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM trace_span_payloads WHERE span_id = $1")
            .bind(&off_span_id)
            .fetch_one(storage.pool())
            .await
            .unwrap();
    assert_eq!(off_payload_count, 0);
    let off_body_count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM trace_payloads WHERE body::TEXT LIKE $1")
            .bind(format!("%{off_marker}%"))
            .fetch_one(storage.pool())
            .await
            .unwrap();
    assert_eq!(off_body_count, 0);

    let (only_session, only_turn) = create_turn(&storage, "session-payload-compaction-only").await;
    let only_request = request("compaction-only-request");
    let ordinary_id = unique("span-ordinary");
    let compaction_id = unique("span-compaction");
    let child_id = unique("span-summary");
    let mut compaction_root = compaction(&only_session, only_turn.as_str(), compaction_id.clone());
    compaction_root.turn_id = Some(only_turn.clone());
    let only_recorder = PostgresTraceRecorder::spawn_with_content_config(
        storage.pool().clone(),
        TraceContentConfig::new(TraceContentPolicy::CompactionOnly, 1_024, 30).unwrap(),
    );
    only_recorder.record(finished_model(
        model_started(
            &only_session,
            Some(&only_turn),
            only_turn.as_str(),
        ...
        Some(json!({"text": "ordinary"})),
    ));
    only_recorder.record(finish_compaction(compaction_root));
    only_recorder.record(finished_model(
        model_started(
            &only_session,
            Some(&only_turn),
        ...
        Some(json!({"text": "summary"})),
    ));
    assert!(only_recorder.flush_turn(&only_turn).await.flushed);
    let mappings: Vec<(String, i64)> = sqlx::query_as(
        "SELECT span.id, COUNT(mapping.slot)::BIGINT
         FROM trace_spans span
         LEFT JOIN trace_span_payloads mapping ON mapping.span_id = span.id
         WHERE span.id IN ($1, $2, $3)
         GROUP BY span.id ORDER BY span.id",
    )
    .bind(&ordinary_id)
    .bind(&compaction_id)
    .bind(&child_id)
    ...
    assert_eq!(
        mappings
            .iter()
            .find(|(id, _)| id == &compaction_id)
            .unwrap()
            .1,
        0
    );
    assert_eq!(
        mappings.iter().find(|(id, _)| id == &child_id).unwrap().1,
        4
    );

    storage.delete_session(&off_session).await.unwrap();
    storage.delete_session(&only_session).await.unwrap();
}

#[tokio::test]
async fn content_policy_changes_apply_to_subsequent_recorder_writes_without_restart() {
    let Some(storage) = storage().await else {
        return;
    };
    let (session_id, turn_id) = create_turn(&storage, "session-payload-runtime-policy").await;
    let recorder = PostgresTraceRecorder::spawn_with_content_config(
        storage.pool().clone(),
        TraceContentConfig::new(TraceContentPolicy::Full, 1_024, 30).unwrap(),
    );

    let full_id = unique("span-runtime-full");
    recorder.record(finished_model(
        model_started(
            &session_id,
            Some(&turn_id),
            ...
            None,
            full_id.clone(),
            &request("runtime-full"),
        ),
        None,
    ));
    assert!(recorder.flush_turn(&turn_id).await.flushed);
    assert!(
        storage
            .get_span_payload(&full_id, TracePayloadSlot::Request)
            .await
            .unwrap()
            .is_some()
    );

    recorder.set_content_policy(TraceContentPolicy::Off);
    assert_eq!(recorder.content_policy(), TraceContentPolicy::Off);
    let off_id = unique("span-runtime-off");
    recorder.record(finished_model(
        model_started(
            &session_id,
            Some(&turn_id),
        ...
        Some(json!({"text": "not stored"})),
    ));
    assert!(recorder.flush_turn(&turn_id).await.flushed);
    let off_payloads: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM trace_span_payloads WHERE span_id = $1")
            .bind(&off_id)
            .fetch_one(storage.pool())
            .await
            .unwrap();
    assert_eq!(off_payloads, 0);

    recorder.set_content_policy(TraceContentPolicy::CompactionOnly);
    assert_eq!(
        recorder.content_policy(),
        TraceContentPolicy::CompactionOnly
    );
    let ordinary_id = unique("span-runtime-ordinary");
    let summary_id = unique("span-runtime-summary");
    recorder.record(finished_model(
        model_started(
            &session_id,
            Some(&turn_id),
    ...
    assert!(
        storage
            .get_span_payload(&ordinary_id, TracePayloadSlot::Request)
            .await
            .unwrap()
            .is_none()
    );
    assert!(
        storage
            .get_span_payload(&summary_id, TracePayloadSlot::Request)
            .await
            .unwrap()
            .is_some()
    );
