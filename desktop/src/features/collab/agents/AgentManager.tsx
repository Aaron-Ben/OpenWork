  ...
  return (
    <section className="min-w-0 flex-1 overflow-y-auto bg-paper">
      <header data-tauri-drag-region="deep" className="flex h-12 items-center justify-between border-b border-line px-6">
        ...
        <Button type="button" size="sm" onClick={() => setForm(emptyAgent())}>
        ...
        <div className="absolute inset-0 z-30 grid place-items-center bg-black/30 p-6" role="dialog" aria-modal="true">
          <form className="grid w-full max-w-xl gap-3 rounded-3xl bg-paper p-6 shadow-xl" onSubmit={submit}>
            ...
            <Field label={t('collab.agents.role')}>
              <Input value={form.role ?? ''} onChange={(event) => setForm({ ...form, role: event.target.value || null })} />
            </Field>
            <Field label={t('collab.agents.mainModel')}>
              <Input required value={form.mainModelId} onChange={(event) => setForm({ ...form, mainModelId: event.target.value })} />
            </Field>
            <Field label={t('collab.agents.triageModel')}>
              <Input required value={form.triageModelId} onChange={(event) => setForm({ ...form, triageModelId: event.target.value })} />
            </Field>
            <Field label={t('collab.agents.persona')}>
              <Textarea required rows={5} value={form.persona} onChange={(event) => setForm({ ...form, persona: event.target.value })} />
            </Field>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button type="submit" variant="accent" disabled={!form.displayName.trim() || !form.mainModelId.trim() || !form.triageModelId.trim()}>{t('collab.agents.save')}</Button>
            </div>
          </form>
        </div>
      ) : null}
