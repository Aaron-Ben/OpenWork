...
export function BoardPage({ rooms }: { rooms: CollabRoom[] }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    await createBoard(roomId, title.trim())
    setTitle('')
    setCreating(false)
  }
  ...
  return (
    <section className="min-w-0 flex-1 overflow-y-auto bg-paper">
      <header data-tauri-drag-region="deep" className="flex h-12 items-center justify-between border-b border-line px-6">
        ...
        <Button type="button" size="sm" disabled={rooms.length === 0} onClick={() => setCreating(true)}>
        ...
        {boards.map((board) => (
          <article key={board.id} className="grid gap-3">
            <div>
              ...
              <p className="text-xs text-ink-faint">{board.id} · {board.roomId}</p>
            </div>
                    ...
                    {column.cards.map((card) => (
                      <article key={card.id} className="rounded-xl border border-line bg-paper p-3 shadow-sm">
                        ...
                        <span className="mt-1 block text-xs text-ink-faint">
                          {card.claimedBy ? `${t('collab.boards.claimedBy')} @${card.claimedBy}` : card.assigneeId ? `${t('collab.boards.assignedTo')} @${card.assigneeId}` : t('collab.boards.unassigned')}
                        </span>
                      </article>
                    ))}
                  </div>
        ...
        <section className="grid gap-2 border-t border-line pt-5">
          ...
          {runs.slice(0, 20).map((run) => (
            <article key={run.id} className="rounded-xl border border-line px-3 py-2 text-sm">
              <strong>@{run.agentId}</strong> · {run.trigger} · {run.status}{run.outcome ? ` · ${run.outcome}` : ''}
        ...
        <div className="absolute inset-0 z-30 grid place-items-center bg-black/30 p-6" role="dialog" aria-modal="true">
          <form className="grid w-full max-w-md gap-3 rounded-3xl bg-paper p-6 shadow-xl" onSubmit={submit}>
            ...
            <label className="grid gap-1 text-xs font-medium text-ink-muted">
              <span>{t('collab.boards.room')}</span>
              <select className="h-9 rounded-md border border-line bg-paper px-3 text-sm" value={roomId} onChange={(event) => setRoomId(event.target.value)}>
                {rooms.map((room) => <option key={room.id} value={room.id}>{room.title ?? room.id}</option>)}
              </select>
            </label>
            <label className="grid gap-1 text-xs font-medium text-ink-muted">
