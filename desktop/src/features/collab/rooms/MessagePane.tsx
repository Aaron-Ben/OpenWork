          ...
          <section className="grid w-full max-w-md gap-4 rounded-3xl bg-paper p-6 shadow-xl">
            ...
            {availableAgents.length > 0 ? (
              <div className="flex gap-2">
                <select className="h-9 min-w-0 flex-1 rounded-md border border-line bg-paper px-3 text-sm" value={agentToAdd} onChange={(event) => setAgentToAdd(event.target.value)}>
                  <option value="">{t('collab.rooms.selectAgent')}</option>
                  {availableAgents.map((agent) => <option key={agent.id} value={agent.id}>{agent.displayName} (@{agent.id})</option>)}
                </select>
